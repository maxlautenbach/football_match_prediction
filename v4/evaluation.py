...
def main():
    ...
    # Evaluate only BL1; feature engineering remains handled by artifacts (may include BL2)
    if "Liga" in test_df.columns:
        before = len(test_df)
        test_df = test_df[test_df["Liga"].astype(str).str.lower() == "bl1"].copy()
        print(f"Filtered Liga=bl1: {len(test_df)}/{before} samples")
    
    # Separate features and target
    print("\nPreparing data...")
    # Store the actual results before removing them
    y_true = test_df["Ergebnis"].copy().reset_index(drop=True)
